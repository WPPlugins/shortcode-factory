<?php $screen_id = $_REQUEST['screen_id']; ?>
<div class="scf-shortcode-wrapper">
	<h2 class="scf-shortcode-title" data-shortcode="scf-post-content">[scf-post-content]</h2>
	<p class="scf-shortcode-desc"><?=__('Outputs content of the post. If ID or Slug is provided, outputs the content of that particular post.', 'shortcode-factory')?></p>

	<?php if($screen_id == 'scf-templates') { ?>
		<p class="scf-notice scf-warning highlight icon">
			For current post, this short code may produce unexpected results, when used in templates (i.e. duplicated content or spitting out the short code itself).
			<br><br>
			<strong>It is highly recommended to target another post, by using ID or Slug attribute.</strong>
		</p>
	<?php }	?>
